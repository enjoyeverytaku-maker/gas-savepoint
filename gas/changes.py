...
人間の手動ゲートを設けていたが、一時期は会長より「セーブポイントを作る際に
それまでの変更の影響についてAIレビューしてくれるように」との指示を受け、
個々の変更への手動レビュー・承認を廃止していた。以降このモジュールは
「いつ・何が変わったか」を検知して記録するだけの受動的なログであり、
実際の影響レビュー（AI／gas/ai_review.py）とセーブ申請の束ね方はセーブ申請時
（gas/releases.py::request_release）に行う。2026-09-16、萬年様向け見積項目3
「変更内容のレビュー・承認フロー」対応として、リリース（個々の変更ではなく
束ねた申請単位）に対する人間の承認ゲートを復活させた（gas/releases.py参照）。
このモジュール自体（個々の変更の検知ログ）は変更なし。
"""
